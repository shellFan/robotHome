<template>
  <div class="rh-card">
    ...
    <div v-else-if="!list.length" class="rh-empty">暂无收藏</div>
      ...
      <div v-for="item in list" :key="item.id" class="item-row">
        <router-link :to="item.url || '#'" class="item-row__main">
          ...
          <div>
            ...
            <div v-if="item.subtitle" class="item-row__sub rh-text-sub">{{ item.subtitle }}</div>
          </div>
        </router-link>
...
async function load () {
  loading.value = true
  try {
    const data = await userApi.myFavorites({
...
async function remove (item) {
  await favoriteApi.remove(item.bizType, item.bizId)
  ElMessage.success('已取消收藏')
  load()
}

onMounted(() => {
  setPageMeta({ title: '我的收藏 - 机器人之家' })
  load()
})
</script>
