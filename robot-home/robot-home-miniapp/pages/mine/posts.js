const { communityApi, userApi } = require('../../api/index')
const { fromNow, unwrapList } = require('../../utils/format')
Page({
  data: { list: [], page: 1, finished: false, loading: false },
  onLoad() { this.reload() },
  onReachBottom() { this.more() },
  async reload() { this.setData({ page: 1, list: [], finished: false }); await this.fetch() },
  async more() {
    if (this.data.loading || this.data.finished) return
    this.setData({ page: this.data.page + 1 })
    await this.fetch(true)
  },
  async fetch(append) {
    this.setData({ loading: true })
    try {
      let data
      try { data = await communityApi.myPosts({ page: this.data.page, size: 10 }) }
      catch (e) { data = await userApi.myPosts({ page: this.data.page, size: 10 }) }
      const records = unwrapList(data).map(function (p) {
        return Object.assign({}, p, { timeText: fromNow(p.createTime) })
      })
      const list = append ? this.data.list.concat(records) : records
      this.setData({ list: list, finished: records.length < 10, loading: false })
    } catch (e) { this.setData({ loading: false }) }
  },
  goDetail(e) { wx.navigateTo({ url: '/pages/community/post-detail?id=' + e.currentTarget.dataset.id }) }
})
