...
Page({
  ...
  async fetch(append) {
    ...
    try {
      const data = await companyApi.page({ page: this.data.page, size: 20, keyword: this.data.keyword || undefined })
      const records = unwrapList(data).map(function (c) { return Object.assign({}, c, { logo: imageOf(c.logo) }) })
  ...
  goDetail(e) { wx.navigateTo({ url: '/pages/companies/detail?id=' + e.currentTarget.dataset.id }) }
})
